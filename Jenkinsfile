node {
    ...
    def filePath  = 'shipping-deployment.yaml'
    stage('create') {
        script {
            sh 'mkdir -p /var/lib/jenkins/workspace/robo_shop_shipping'
        }
    }
    stage('clone') {
      dir('/var/lib/jenkins/workspace/robo_shop_shipping') {
        git branch: 'shipping', credentialsID: 'git-hub', url: 'https://github.com/prasad1613/robo-shop.git'
    }
    }
    stage('docker build') {
      dir('/var/lib/jenkins/workspace/robo_shop_shipping') {
