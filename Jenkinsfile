node {
    def dockerImage = 'sphari/payment'
    def dockercredentialsID = 'docker'
    def filePath  = 'payments-deployment.yaml'
    stage('create') {
       script {
           sh 'mkdir -p /var/lib/jenkins/workspace/robo_shop_payments'
    }
    }
    stage('clone') {
      dir('/var/lib/jenkins/workspace/robo_shop_payments') {
        git branch: 'payments', credentialsID: 'git-hub', url: 'https://github.com/prasad1613/robo-shop.git'
    }
    }
    stage('docker build') {
      dir('/var/lib/jenkins/workspace/robo_shop_payments') {
    ...
    stage('replace version') {
        ...
        def dockerImageTag = "${dockerImage}:${env.BUILD_NUMBER}"
        script {
            sh "sed -i 's|sphari/payment:[0-9.]*|${dockerImageTag}|g' ${filePath}"
        }    
        }
    }
    stage('push version in github') {
        ...
        sh "git add ${filePath}"
        sh "git commit -m change-in-payments-deployment"
    }
}  
