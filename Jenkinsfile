    ...
    def filePath  = 'mongo-deployment.yaml'
    stage('create') {
        script {
            sh 'mkdir -p /var/lib/jenkins/workspace/robo_shop_mongo'
        }
    }
    stage('clone') {
     dir('/var/lib/jenkins/workspace/robo_shop_mongo') {
    ...
    stage('replace version') {
        ...
        def dockerImageTag = "${dockerImage}:${env.BUILD_NUMBER}"
        script {
            sh "sed -i 's|sphari/mongodb:[0-9.]*|${dockerImageTag}|g' ${filePath}"
        }    
        }
    }
    stage('push version in github') {
        ...
        sh "git add ${filePath}"
        sh "git commit -m change-in-mongo-deployment"
        sh "git pull origin main"
    }
}  
